...
Суперпользователь будет автоматически создаваться при каждом запуске контейнера с такими данными которые вы укажете в переменных   
Volumes из-за этого в docker я делать не стал, чтобы не было ошибки при перезапуске о том что такой пользователь уже существует для простоты просмотра вами   
в реальной версии приложения суперпользователя я бы создавал однажды например через docker exec и изнутри контейнера с приложением с помощью py manage.py createsuperuser.   
